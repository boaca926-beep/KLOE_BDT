...
int tree_cut_bdt_raw() {
    ...
    // Correction variables (unused in raw version, but kept to avoid changing branch structure)
	...
	// Photon energies: use the kinematic fit values directly (no mass scale, no bias shift)
	e1_bdt = e1_fit;
	e2_bdt = e2_fit;
	e3_bdt = e3_fit;
	...
	// Set correction variables to 0 (they are not used)
	delta_e1_bdt = 0.;
	delta_e2_bdt = 0.;
	e1_pull_bdt_new = 0.;
	e2_pull_bdt_new = 0.;
	// Pulls will be computed later based on these values.

	// ---------- Continue with computed quantities ----------
	m_gg_bdt = compute_invariant_mass(result.pi0_indices[0], result.pi0_indices[1], event.photons);
        ...
        // Compute pulls (corrected - true) – here corrected = raw, so pulls are raw - true
