...
int tree_cut_bdt_scaled() {
	...
	// --- ADD THESE: BDT corrected photon momenta ---
	...
	*/
	...
	const double resol_E12 = (resol_E1 + resol_E2) / 2.;
	...
	if (data_type == "sig") {
	  // ============================================================
	  // SIMPLE SCALING ONLY – NO PULL TUNING
	  // ============================================================
	  // Scale π⁰ photons (E1, E2) and their momenta
	  e1_bdt = e1_raw * MASS_SCALE_PI0;
	  e2_bdt = e2_raw * MASS_SCALE_PI0;
	  e3_bdt = e3_raw;   // ISR photon – unchanged
	  
	  // Scale momenta for the π⁰ photons
	  px1_bdt = px1_raw * MASS_SCALE_PI0;
	  py1_bdt = py1_raw * MASS_SCALE_PI0;
	  pz1_bdt = pz1_raw * MASS_SCALE_PI0;
	  
	  px2_bdt = px2_raw * MASS_SCALE_PI0;
	  py2_bdt = py2_raw * MASS_SCALE_PI0;
	  pz2_bdt = pz2_raw * MASS_SCALE_PI0;
	  
	  // ISR momenta – unchanged
	  px3_bdt = px3_raw;
	  py3_bdt = py3_raw;
	  pz3_bdt = pz3_raw;
	  
	  // Update event.photons with the scaled values
	  ...
	  // --- IMPORTANT: Copy raw momenta for data ---
	  px1_bdt = px1_raw;
	  py1_bdt = py1_raw;
	  pz1_bdt = pz1_raw;
	...
	// e_asy
	e_asym = TMath::Abs(e1_bdt - e2_bdt) / (e1_bdt + e2_bdt);
