// correct_and_plot_single_sideband_chebyshev_2ndorder.C
// Single lower‑sideband Chebyshev fit (2nd order) with full plotting
...
Double_t template_chebyshev2(Double_t *x, Double_t *par) {
    ...
    Double_t X = x[0];
    Double_t cheb = par[1] + par[2]*X + par[3]*(2*X*X - 1);
    return par[0] * sig + cheb;
}

void correct_and_plot_single_sideband_chebyshev_2ndorder() {
    ...
    // ------------------------------------------------------------------
    // 6. Single lower‑sideband Chebyshev fit (2nd order)
    // ------------------------------------------------------------------
    ...
    std::cout << "Single lower‑sideband Chebyshev fit (2nd order):\n"
    ...
    for (int bin = 1; bin <= h_background->GetNbinsX(); ++bin) {
        ...
        double val = cheb0 + cheb1*x + cheb2*(2*x*x - 1);
        if (val < 0) val = 0;
    ...
    // 11. Plotting
    // ------------------------------------------------------------------
    TCanvas *c = new TCanvas("c", "3π mass projection (Chebyshev 2nd order, single sideband)", 1200, 700);
    ...
    c->SaveAs(output_path + "combined_fit_and_plot_single_sideband_chebyshev_2ndorder.pdf");
    delete c;
    ...
    c22->Update();
    c22->SaveAs(output_path + "background_subtracted_single_sideband_chebyshev_2ndorder.pdf");
    delete c22;
    
    // ------------------------------------------------------------------
    // 13. Save output ROOT file
    // ------------------------------------------------------------------
    TFile *fout = new TFile(output_path + "corrected_and_plotted_single_sideband_chebyshev_2ndorder.root", "RECREATE");
...
int main() {
    correct_and_plot_single_sideband_chebyshev_2ndorder();
    return 0;
}
