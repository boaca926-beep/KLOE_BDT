...
int getplothist(const TString plot_type = "") {
  ...
  TH2D *h_sig_peak = getHist(Form("h2d_%s_TISR3PI_SIG_peak", prefix.Data()));
  TH2D *h_sig_non_reson = getHist(Form("h2d_%s_TISR3PI_SIG_non_reson", prefix.Data()));
  //h_sig_peak->Draw();
  //h_sig_non_reson->Draw();
  ...
  // Background sum without signal
  TH2D *hbkgsum_nosig = (TH2D*)h_eeg->Clone();
  hbkgsum_nosig->Add(h_ksl, 1.);
  hbkgsum_nosig->Add(h_omega, 1.);
  hbkgsum_nosig->Add(h_etagam, 1.);
  hbkgsum_nosig->Add(hmcrest, 1.);
  
  // MC sum (background + signal)
  TH2D *hmcsum = (TH2D*)hbkgsum_nosig->Clone();
  hmcsum->Add(h_sig, 1.);
  
  hmcrest->SetName("hmcrest");
  hbkgsum_nosig->SetName("hbkgsum_nosig");
