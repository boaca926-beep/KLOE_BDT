// correct_and_plot.C
// Combined macro:
//   1. Reads trees from tree_pre_bdt.root.
//   2. Creates data and MC background histograms (200 bins, 600‑1000 MeV).
//   3. Performs template fit: α * signal_template (correctly paired ω) + linear polynomial
//      using only lower sideband (690‑710 MeV) for background.
//   4. Corrects ISR3pi MC shape via weights.
//   5. Produces stacked plot with pulls (full range 600‑1000 MeV).
//   6. Saves output ROOT file and PDFs.
...
// Global pointer for signal template (needed by fit function)
...
void correct_and_plot() {
    ...
    // Fixed binning (must match correction histograms)
    ...
    // MC Rest (TKPM, TRHOPI, TBKGREST) – set attributes manually
    ...
    // ------------------------------------------------------------------
    // 4. Subtract MC backgrounds from data → h_data_isr (for fit)
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // 6. Fit: lower sideband linear polynomial + signal template
    // ------------------------------------------------------------------
    double peak_low = 740.0;
    double peak_high = 820.0;
    double sb_low = 690.0;
    double sb_high = 710.0;

    // Clone for sideband fit
    ...
    std::cout << "Linear sideband fit (" << sb_low << "-" << sb_high << " MeV): "
              << p0 << " + " << p1 << "·x\n";

    // Total fit function (3 parameters: alpha, p0, p1)
    TF1 *total_func = new TF1("total_func", template_linear, mass_min, mass_max, 3);
    ...
    std::cout << "Fit results: α = " << alpha << ", linear: " << poly0 << " + " << poly1 << "·x\n";
    ...
    // ------------------------------------------------------------------
    // 9. Build total MC sum for plotting (using original MC components)
    // ------------------------------------------------------------------
    ...
    leg->AddEntry(h_background, "Combinatorial", "l");
    ...
    c->SaveAs(output_path + "combined_fit_and_plot.pdf");
    delete c;
    ...
    leg2->AddEntry(h_background, "Combinatorial", "l");
    leg2->Draw();
    c2->SaveAs(output_path + "background_subtracted_combined.pdf");
    delete c2;

    // ------------------------------------------------------------------
    // 13. Save output ROOT file
    // ------------------------------------------------------------------
    TFile *fout = new TFile(output_path + "corrected_and_plotted.root", "RECREATE");
    ...
    // ------------------------------------------------------------------
    // 14. Clean up (critical: detach total_func before deleting)
    // ------------------------------------------------------------------
    total_func->SetParent(0);   // detach from h_data_isr
    delete total_func;          // now safe
    delete bkg_linear;
    delete h_side;
    ftree->Close();
    delete ftree;
    delete fout;
    gSigTemplate = nullptr;     // reset global
