...
void omega_fit_bdt_linear() {
    ...
    // ------------------------------------------------------------------
    double fit_low = 700;    // expanded fit range
    double fit_high = 850;
    ...
    // ------------------------------------------------------------------
    // 17. Clean up (SAFE - no double deletion)
    // ------------------------------------------------------------------
    gSigTemplate = nullptr;
    gBkgTemplate = nullptr;
    
    // Delete fit functions and sideband hist
    if (total_func) delete total_func;
    if (bkg_linear) delete bkg_linear;
    if (h_side) delete h_side;
    
    // Delete canvases (pads are children and will be deleted with canvas)
    if (c) delete c;
    if (c2) delete c2;
    if (c_weight) delete c_weight;
    
    // Delete histograms (only those not owned by canvases or files)
    ...
    // DO NOT delete pad1, pad2 - they are children of canvas c
