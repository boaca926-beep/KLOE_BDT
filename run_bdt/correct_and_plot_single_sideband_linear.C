// correct_and_plot_single_sideband_linear.C
// Single lower‑sideband linear fit with full plotting
...
// Fit function: α * signal_template + linear polynomial (p0 + p1*x)
Double_t template_linear(Double_t *x, Double_t *par) {
    ...
    Double_t poly = par[1] + par[2]*X;
    return par[0] * sig + poly;
}

void correct_and_plot_single_sideband_linear() {
    ...
    // ------------------------------------------------------------------
    // 6. Single lower‑sideband linear fit
    // ------------------------------------------------------------------
    ...
    // Linear polynomial: p0 + p1*x
    TF1 *bkg_linear = new TF1("bkg_linear", 
        "[0] + [1]*x", 
        sb_low, sb_high);
    ...
    std::cout << "Single lower‑sideband linear fit:\n"
    ...
    // 11. Plotting
    // ------------------------------------------------------------------
    TCanvas *c = new TCanvas("c", "3π mass projection (linear fit, single sideband)", 1200, 700);
    ...
    leg->AddEntry(h_background, "Non-resonant background (linear)", "l");
    ...
    c->SaveAs(output_path + "combined_fit_and_plot_single_sideband_linear.pdf");
    delete c;
    ...
    leg2->AddEntry(h_background, "Non-resonant background (linear)", "l");
    leg2->Draw();

    c22->Update();
    c22->SaveAs(output_path + "background_subtracted_single_sideband_linear.pdf");
    delete c22;
    
    // ------------------------------------------------------------------
    // 13. Save output ROOT file
    // ------------------------------------------------------------------
    TFile *fout = new TFile(output_path + "corrected_and_plotted_single_sideband_linear.root", "RECREATE");
...
int main() {
    correct_and_plot_single_sideband_linear();
    return 0;
}
